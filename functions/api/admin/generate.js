...
export async function onRequest(context) {
    ...
    // --- UPDATED LOGIC ---
    // 1. Start the sermon generation in the background using waitUntil.
    //    This allows the function to continue running even after the
    //    response is sent to the client.
    console.log('Triggering sermon generation...');
    waitUntil(generateAndStoreSermon(env));

    // 2. Immediately return a success response to the admin panel.
    //    The user no longer has to wait for the API call to finish.
    return new Response(JSON.stringify({ 
        success: true, 
        message: 'Sermon generation has been successfully triggered. It will be available shortly.' 
    }), {
        status: 200, // Use 202 (Accepted) if you prefer, but 200 is fine.
        headers: { 'Content-Type': 'application/json' }
    });
}

/**
 * This function now runs in the background, triggered by `waitUntil`.
 * It will fetch the sermon from the Gemini API and store it.
 */
async function generateAndStoreSermon(env) {
    console.log("Starting background sermon content generation...");
...
Your response MUST be a JSON object with this exact schema:
{
  ...
  "text": "The complete sermon text with proper paragraph breaks using \\n\\n"
    ...
    // --- REMOVED TIMEOUT ---
    // The AbortController and setTimeout have been removed.
    // The function will now wait for the Gemini API to respond,
    // which is fine because it's running in the background.
    ...
    try {
        const response = await fetch(GEMINI_URL, {
            ...
            // The 'signal' property is removed
        });

        if (!response.ok) {
            const errorText = await response.text();
            console.error('Gemini API Error Response:', errorText);
            // Since this is in the background, we log the error.
            // We can't return an HTTP 500 to the original client.
        ...
        console.log("Background sermon generation successful. Stored with ID:", sermonId);

    } catch (e) {
        // Log any errors from the background task
        console.error("Failed to generate or store sermon in background:", e);
